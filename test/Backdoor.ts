...
describe("[Challenge] Backdoor", function () {
  let deployer: any, users: any, player: any, alice, bob, charlie, david;
  let masterCopy: any, walletFactory: any, token: any, walletRegistry: any;
  ...
  it("Execution", async function () {
    ...
    const walletRegistryAttacker = await (
      await ethers.getContractFactory("WalletRegistryAttacker")
    ).deploy(
      await token.getAddress(),
      await walletFactory.getAddress(),
      await walletRegistry.getAddress()
    ...
    await walletRegistryAttacker.connect(player).exploit(users);
  });
